...
class Git {
  ...
  static void init() {
    ...
    // HttpHeaders.authorizationHeader 为string类型的authorization
    // Global.profile.token即为json转出的model中的profile中的token值
    dio.options.headers[HttpHeaders.authorizationHeader] = Global.profile.token;

    // 在调试模式下需要抓包调试，所以我们使用代理，并禁用HTTPS证书校验
    // 在全局环境下做了一次是否为正式环境的校验, 如果不是release版本, 则...
    if (!Global.isRelease) {
      // 创建httpClient时, 设置一个proxy
